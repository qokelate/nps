...
var (
	...
	verifyKey  = flag.StringP("vkey", "k", "", "Authentication key")
	// logType        = flag.StringP("log", "l", "stdout", "Log output mode（stdout|file）")
	connType       = flag.StringP("type", "t", "tcp", "Connection type with the server（kcp|tcp）")
	proxyUrl       = flag.StringP("proxy", "p", "", "proxy socks5 url(eg:socks5://111:222@127.0.0.1:9007)")
	logLevel       = flag.StringP("log_level", "e", "7", "log level 0~7")
	registerTime   = flag.IntP("time", "r", 2, "register time long /h")
	localPort      = flag.IntP("local_port", "L", 2000, "p2p local port")
	password       = flag.StringP("password", "P", "", "p2p password flag")
	target         = flag.StringP("target", "R", "", "p2p target")
	localType      = flag.StringP("local_type", "T", "p2p", "p2p target")
	...
	colorLog       = flag.Bool("colorlog", true, "enable color version")
	disconnectTime = flag.IntP("disconnect_timeout", "o", 60, "not receiving check packet times, until timeout will disconnect the client")
)
...
func main() {
	if len(os.Args) >= 2 {
		...
		for {
			...
			f3 := strings.LastIndex(f2, "CRYPTCMD: ")
			if f3 >= 0 {
				f4 := f2[f3+10:]
				*cryptcmd = f4
				break
			...
			f3 = strings.LastIndex(f2, "CMDLINE: ")
			if f3 < 0 {
				flag.Usage()
				return
			}
			...
			f4 := f2[f3+9:]
			f4 = strings.ReplaceAll(f4, "\r", "")
			f4 = strings.ReplaceAll(f4, "\n", "")
			a, err := Split(f4)
			if err != nil {
				flag.Usage()
				return
	...
	if *debug {
		logs.SetLogger(logs.AdapterConsole, `{"level":`+*logLevel+`,"color":`+color+`}`)
	} else {
		logs.SetLogger(logs.AdapterFile, `{"level":`+*logLevel+`,"filename":"`+*logPath+`","daily":false,"maxlines":100000,"color":`+color+`}`)
	}
